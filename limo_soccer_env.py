...
Apprentissage hiérachique avec placement de la balle devant la voiture pour les 5M premiers steps
Balle devant mais plus loin de 5M à 10M
Balle placé aléatoirement de 10M à 20M
"""
...
class LimoSoccerEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = "human"):
        ...
        # Modifier la difficulté au fur et à mesure des steps d'apprentissage
        self.total_steps = 0
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[np.ndarray, dict]:
        ...
        # -------- Partie Apprentissage Hiérachique --------
        if self.total_steps < 5_000_000:
            self._reset_ball_center(mode="easy")
        elif self.total_steps < 10_000_000:
            self._reset_ball_center(mode="medium")
        else:
            self._reset_ball_center(mode="normal")
    ...
    # ---- placement de la balle avec version hiérachique ----
    def _reset_ball_center(self, mode="normal"):
        if mode == "easy":
            # balle devant la voiture
            offset = np.array([
                math.cos(self.car_angle),
                math.sin(self.car_angle)
            ]) * 100.0

            self.ball_pos[:] = self.car_pos + offset
            self.ball_vel[:] = 0.0
    ...
    def _simulate_physics(self):
        ...
        # vittesse et position de la balle
